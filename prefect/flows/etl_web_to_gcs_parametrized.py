...
"""Parametrizing the Flow
So that the flow can be run multiple times 
with different parameters passed during runtime.
"""
...
# Create a parent flow where we can pass a list of months and run the sub flow for each month
@flow()
def etl_parent_flow(year: int, months: list[int]=[1,2],  color: str="yellow"):
    for month in months:
